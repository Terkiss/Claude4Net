...
- 본 에이전트는 단발성 최종 관제 판단을 수행하는 도구입니다. 최종 승인 여부, 지시문 작성 및 커밋 결과 보고가 완료되어 호출자(오케스트레이터)가 이를 완전히 접수한 것이 확인되면, 호출자는 `manage_subagents` 도구의 `kill` 액션을 통해 본 세션과 분기 워크스페이스를 즉각 폐기해야 합니다. (단, 결과 보고와 산출물 저장 완료 전 조기 kill 금지)
