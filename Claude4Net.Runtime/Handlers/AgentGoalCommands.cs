...
namespace Claude4Net.Runtime.Handlers
{
    public static class AgentGoalCommands
    {
        ...
        public static async Task<string> HandleSave(string a, IServiceProvider sp)
        {
            try
            {
                if (string.IsNullOrEmpty(AppState.CurrentCwd))
                {
                    return "[red]Error:[/] Workspace is not set. Use [bold]/setworkspace <path>[/] before saving context.";
                }
                ...
                try
                {
                    ...
                    provider = registry.CreateProvider(AppState.ActiveProvider, sp);
                }
                catch
                {
                    if (AppState.ActiveProvider == "gemini") provider = sp.GetRequiredService<GeminiProvider>();
                    else if (AppState.ActiveProvider == "ollama") provider = sp.GetRequiredService<OllamaProvider>();
                    else provider = sp.GetRequiredService<ClaudeService>();
                }
                ...
                string json = System.Text.Json.JsonSerializer.Serialize(history, new System.Text.Json.JsonSerializerOptions { WriteIndented = true });
                await File.WriteAllTextAsync(fullPath, json);

                return $"[green]Context saved to:[/] [underlined]{Markup.Escape(fullPath)}[/]\nConversation context saved to {fileName}";
            }
            catch (Exception ex)
            {
                return $"[red]Error saving context:[/] {Markup.Escape(ex.Message)}";
            }
        }
    }
}
