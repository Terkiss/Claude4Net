...
{
    [Collection("AppState")]
    public class OfficialOpenAiJsSdkBlackBoxIntegrationTests : IAsyncLifetime
    {
        private ServiceProvider _serviceProvider = null!;
        private Claude4NetApiServer _server = null!;
        private int _testPort;
        private const string TestApiKey = "c4n-sk-js-official-sdk-key-7777";
        ...
        public async Task InitializeAsync()
        {
            ...
            services.AddSingleton<IProviderFactory, JsSdkBlackBoxMockProviderFactory>();
        ...
        [Fact]
        public async Task FinalControl_OfficialOpenAiJavaScriptSdk_ExecutesCompleteBlackBoxSuite()
        {
            const string successMarker = "ALL 12/12 OFFICIAL OPENAI JS SDK BLACK-BOX TESTS PASSED!";
            var request = new SdkProcessRequest(
                SdkRuntime.Node,
                "blackbox_openai_js_sdk_runner.mjs",
                _testPort,
                TestApiKey,
                successMarker);
        ...
        private class JsSdkBlackBoxMockProviderFactory : IProviderFactory
        {
            ...
            {
                return new JsSdkBlackBoxMockProvider();
            }
        ...
        private class JsSdkBlackBoxMockProvider : ILLMProvider
        {
            private readonly System.Collections.Generic.List<object> _history = new();
            public string Name => "JsSdkBlackBoxMockProvider";
            public ITokenCounter TokenCounter { get; } = new JsSdkBlackBoxTokenCounter();
            ...
            public async System.Collections.Generic.IAsyncEnumerable<LLMStreamEvent> StreamQueryAsync(string prompt, string? model = null, [EnumeratorCancellation] CancellationToken ct = default)
            {
                ...
                {
                    yield return new LLMStreamEvent { Type = LLMStreamEventType.TextDelta, Delta = "<think>Step 1: JS Reasoning</think>JS Final Calculation Complete." };
                }
                else
                {
                    yield return new LLMStreamEvent { Type = LLMStreamEventType.TextDelta, Delta = "Mock JS blackbox response for: " + prompt };
                }
            }
        ...
        private class JsSdkBlackBoxTokenCounter : ITokenCounter
        {
